...
void test_validate_my_username()
{
    ...
    // Call malloc_username_from_conf_file() to get the username from the config file
    char* conf_file_username = malloc_username_from_conf_file();

    // Use Unity assertion to verify the two strings are equal
    TEST_ASSERT_EQUAL_STRING_MESSAGE(hard_coded_username, conf_file_username, "Usernames do not match!");
